...
class KronosBacktester:
    ...
    def align_data(self, hist_df, pred_df):
        ...
        print(
            f"✅ 数据对齐: 历史数据结束于 {last_hist_date}, 预测数据从 {pred_df_aligned.index.min()} 开始"
        )
    ...
    def calculate_trading_signals(self, hist_df, pred_df, threshold=0.02):
        ...
        combined = pd.concat(
            [
                ...
                pred_df[["predicted_close"]].rename(
                    columns={"predicted_close": "predicted"}
                ),
            ],
            axis=1,
        )
        ...
        # 过滤信号：避免频繁交易
        combined["position"] = (
            combined["signal"].replace(to_replace=0, method="ffill").fillna(0)
        )
    ...
    def run_backtest(self, combined_df):
        ...
        backtest_results["price"] = combined_df["actual"].combine_first(
            combined_df["predicted"]
        )

        for i, (date, row) in enumerate(combined_df.iterrows()):
            current_price = (
                row["actual"] if not pd.isna(row["actual"]) else row["predicted"]
            )
            signal = row["position"]
            ...
            if i > 0:
                ...
                if prev_value > 0:
                    backtest_results.loc[date, "returns"] = (
    ...
    def calculate_metrics(self, backtest_results, trades):
        ...
        """
        returns = (
            backtest_results["returns"].replace([np.inf, -np.inf], np.nan).dropna()
        )
        ...
        total_return = (
            backtest_results["capital"].iloc[-1] - self.initial_capital
    ...
    def plot_backtest_results(self, backtest_results, metrics, stock_code, output_dir):
        ...
        drawdown = (cumulative_returns - peak) / peak
        ax3.fill_between(
            backtest_results.index, drawdown, 0, alpha=0.3, color="red", label="回撤"
        )
    ...
    def run_complete_backtest(self, stock_code, output_dir, threshold=0.02):
        ...
        try:
            ...
            self.plot_backtest_results(
                backtest_results, metrics, stock_code, output_dir
            )
            ...
            for i, trade in enumerate(trades[-10:], 1):  # 显示最后10次交易
                print(
                    f"  交易{i}: {trade['date'].strftime('%Y-%m-%d')} "
...
def main():
    ...
    BACKTEST_CONFIG = {
        ...
        "data_dir": "examples/data",  # 历史数据目录
        "model_dir": "examples/yuce",  # 模型预测结果目录
        "output_dir": "examples/backtest",  # 回测结果输出目录
