...
class StockPredictorGUI:
    ...
    def create_widgets(self):
        ...
        stock_code_entry = tk.Entry(
            input_frame, textvariable=self.stock_code_var, font=("Arial", 10), width=15
        )
        ...
        stock_name_entry = tk.Entry(
            input_frame, textvariable=self.stock_name_var, font=("Arial", 10), width=15
        )
        ...
        pred_days_entry = tk.Entry(
            input_frame, textvariable=self.pred_days_var, font=("Arial", 10), width=15
        )
        ...
        history_years_entry = tk.Entry(
            input_frame,
            textvariable=self.history_years_var,
            font=("Arial", 10),
            width=15,
        )
        ...
        # 数据目录
        tk.Label(dir_frame, text="数据目录:", bg="#f0f0f0", font=("Arial", 10)).grid(
            row=0, column=0, sticky=tk.W, padx=5, pady=5
        )
        self.data_dir_var = tk.StringVar(value="examples/data")
        data_dir_entry = tk.Entry(
            dir_frame, textvariable=self.data_dir_var, font=("Arial", 10), width=40
        )
        data_dir_entry.grid(row=0, column=1, padx=5, pady=5)
        tk.Button(
            dir_frame, text="浏览", command=self.browse_data_dir, font=("Arial", 9)
        ).grid(row=0, column=2, padx=5, pady=5)

        # 输出目录
        tk.Label(dir_frame, text="输出目录:", bg="#f0f0f0", font=("Arial", 10)).grid(
            row=1, column=0, sticky=tk.W, padx=5, pady=5
        )
        self.output_dir_var = tk.StringVar(value="examples/yuce")
        output_dir_entry = tk.Entry(
            dir_frame, textvariable=self.output_dir_var, font=("Arial", 10), width=40
        )
        output_dir_entry.grid(row=1, column=1, padx=5, pady=5)
        tk.Button(
            dir_frame, text="浏览", command=self.browse_output_dir, font=("Arial", 9)
        ...
        self.result_text = tk.Text(
            self.result_frame, height=8, font=("Arial", 9), wrap=tk.WORD
        )
    ...
    def run_prediction(self):
        ...
        try:
            ...
            if success:
                ...
                messagebox.showinfo(
                    "完成", f"{stock_name}({stock_code})预测完成！\n图表已保存到输出目录。"
                )
            else:
        ...
        try:
            ...
            if stock_code == "600580":
                fundamentals = {
                    ...
                    "risk_factors": ["机器人业务营收占比仅2.71%，占比较低", "工业需求景气度波动", "原料价格波动风险"],
            ...
            for col in ["close", "open", "high", "low"]:
                if col in adjusted_df.columns:
                    for idx in post_holiday_indices:
                        adjusted_df.iloc[idx][col] = adjusted_df.iloc[idx][col] * (
                            1 + effect_strength
                        )
        ...
        if col in validated_df.columns:
            ...
            for i in range(len(validated_df)):
                ...
                if abs(predicted_price - current_price) > max_allowed_change:
                    ...
                    corrected_price = current_price * (
                        1 + (predicted_price / current_price - 1) * correction_factor
                    )
                    validated_df.iloc[i][col] = corrected_price

                    print(
                        f"⚠️  修正异常{col}价格: {predicted_price:.2f} -> {corrected_price:.2f}"
                    )
...
# ==================== GUI版本预测函数 ====================
def run_comprehensive_prediction_gui(
    stock_code,
    stock_name,
    data_dir,
    pred_days,
    ...
    try:
        ...
        validated_pred_df = validate_prediction_results(
            historical_df_for_validation, pred_df
        )

        # 🎯 新增：对基础预测进行平滑处理
        update_progress("步骤7.5: 对预测结果进行平滑优化...")
        smoothed_pred_df = smooth_prediction_results(
            validated_pred_df, historical_df_for_validation
        )
        ...
        adjusted_pred_df = apply_post_holiday_adjustment(
            smoothed_pred_df, future_dates, holiday_periods
        )
        ...
        if len(enhanced_pred_df) > 0:
            ...
            base_predicted_price = (
                adjusted_pred_df["close"].iloc[-1]
                if len(adjusted_pred_df) > 0
                else current_price
            )
            ...
            update_result(
                f"平滑预测价格: {base_predicted_price:.2f} 元 ({base_change_pct:+.2f}%)"
            )
            update_result(
            ...
            update_result(
                f"大盘状态: {enhancement_info['market_analysis']['market_status']}"
            )
            update_result(
                f"板块共振: {enhancement_info['sector_analysis']['main_sector']['sector']}"
            )
            update_result(
            ...
            prediction_details = pd.DataFrame(
                {
                    "日期": future_dates,
                    "平滑预测收盘价": adjusted_pred_df["close"].values
                    if len(adjusted_pred_df) > 0
...
def plot_optimized_prediction_gui(
    historical_df,
    base_pred_df,
    enhanced_pred_df,
    ...
    fig.suptitle(
        f"{stock_name}({stock_code}) - 优化版交易日预测图表", fontsize=16, fontweight="bold"
    )
    ...
    ax3.plot(
        historical_df["timestamps"],
        historical_df["close"].pct_change() * 100,
        color=colors["historical"],
        linewidth=1.5,
        label="历史涨跌幅",
        alpha=0.7,
