        ...
        if response.status_code == 200:
            ...
            if start_idx != -1 and end_idx != -1:
                ...
                try:
                    ...
                    return parse_kline_data_directly_all_history(
                        response_text, stock_code
                    )
            else:
                print("❌ 无法找到JSON数据边界")
                return None
...
def parse_kline_data_directly_all_history(response_text, stock_code):
    ...
    try:
        ...
        if '"klines":[' in response_text:
            ...
            # 清理字符串并分割
            klines = [
                k.strip().strip('"') for k in klines_str.split('","') if k.strip()
            ]
...
def create_sample_data_all_history(stock_code="002354"):
    ...
    for i, date in enumerate(all_dates):
        ...
        volume = int(
            base_volume + volume_variation + np.random.randint(-200000, 400000)
        )
...
def display_all_history_data_info(df, stock_code):
    ...
    if len(years) > 5:
        key_years.extend(
            [years[len(years) // 2], years[len(years) // 4], years[3 * len(years) // 4]]
        )
