...
def plot_prediction_with_details(
    kline_df,
    pred_df,
    future_dates,
    stock_code="002354",
    ...
    prediction_start_date = (
        close_df["预测数据"].index[0] if len(close_df["预测数据"]) > 0 else close_df.index[-1]
    )
    prediction_start_price = close_df["历史数据"].iloc[-1]
    ax1.axvline(
        x=prediction_start_date, color="red", linestyle="--", alpha=0.7, linewidth=1.5
    )
    ax1.annotate(
        "预测起点",
        xy=(prediction_start_date, prediction_start_price),
        xytext=(10, 10),
    ...
    ax1.set_title(
        f"{stock_name}({stock_code}) 股票价格预测 - 未来{pred_len}个交易日",
        fontsize=16,
        fontweight="bold",
        pad=20,
    )
    ...
    if len(close_df["预测数据"]) > 0:
        ...
        for i, bar in enumerate(bars):
            ...
            if (
                i % 10 == 0 or i == len(bars) - 1 or abs(height) > price_change.std()
            ):  # 每10天或最后一天或显著波动
                ax3.text(
    ...
    if len(price_change) > 0:
        # 每10个交易日显示一个标签
        xticks_positions = list(
            range(0, len(price_change), max(1, len(price_change) // 10))
        )
    ...
    prediction_details = pd.DataFrame(
        {
            ...
            "价格变动(%)": (
                (close_df["预测数据"].values[:min_len] / historical_close - 1) * 100
            ),
        }
    )
...
def main(
    ...
    try:
        ...
        future_dates = generate_future_dates_with_holidays(
            last_historical_date, pred_len
        )
        ...
        close_df, volume_df = plot_prediction_with_details(
            kline_df,
            pred_df,
            future_dates,
            stock_code,
        ...
        print("生成的文件:")
        print(
            f"  📊 {os.path.join(output_dir, stock_code + '_prediction_chart.png')} - 预测图表"
        )
        print(
            f"  📋 {os.path.join(output_dir, stock_code + '_detailed_predictions.csv')} - 详细预测数据"
        )
