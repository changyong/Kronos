        ...
        if response.status_code == 200:
            ...
            if start_idx != -1 and end_idx != -1:
                ...
                try:
                    ...
                    # 如果JSON解析失败，尝试直接提取数据
                    return parse_kline_data_directly(
                        response_text, stock_code, start_year, end_year
                    )
            else:
                print("❌ 无法找到JSON数据边界")
                return None
            ...
            if data and data.get("data") is not None:
                ...
                print(
                    f"实际时间范围: {df.index.min().strftime('%Y-%m-%d')} 到 {df.index.max().strftime('%Y-%m-%d')}"
                )
                return df
            else:
                print("❌ API返回数据为空")
...
def get_stock_data_with_retry(
    stock_code="002354", start_year=2024, end_year=2025, retry_count=2
):
    """
    带重试机制的数据获取 - 多数据源版本
    """
...
def create_sample_data(stock_code="002354", start_year=2024, end_year=2025):
    ...
    for i, date in enumerate(trading_dates):
        ...
        volume = int(
            base_volume + volume_variation + np.random.randint(-100000, 200000)
        )
    ...
    print(
        f"时间范围: {df.index.min().strftime('%Y-%m-%d')} 到 {df.index.max().strftime('%Y-%m-%d')}"
    )
...
def display_data_info(df, stock_code, start_year, end_year):
    ...
    print(
        f"数据时间范围: {df.index.min().strftime('%Y-%m-%d')} 到 {df.index.max().strftime('%Y-%m-%d')}"
    )
    ...
    for year in sorted(df.index.year.unique()):
        ...
        if len(year_data) > 1:
            year_return = (
                year_data["收盘价"].iloc[-1] / year_data["收盘价"].iloc[0] - 1
            ) * 100
...
def save_stock_data(df, stock_code, save_dir="examples/data"):
    """
    保存股票数据到指定目录
    """
    if df is not None and not df.empty:
