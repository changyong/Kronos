...
from finetune_tokenizer import (
    train_tokenizer,
    set_seed,
    setup_logging as setup_tokenizer_logging,
)
...
class SequentialTrainer:
    ...
    def _setup_device(self):
        ...
        if self.rank == 0:
            print(
                f"Using device: {device} (rank={self.rank}, world_size={self.world_size}, local_rank={self.local_rank})"
            )
        return device
    ...
    def _setup_distributed(self):
        if self.world_size > 1 and torch.cuda.is_available():
            ...
            if self.rank == 0:
                print(
                    f"Distributed training initialized: backend={backend}, world_size={self.world_size}"
                )
        else:
            if self.rank == 0:
    ...
    def train_tokenizer_phase(self):
        ...
        if getattr(self.config, "pre_trained_tokenizer", True):
            ...
            if self.rank == 0:
                print(
                    "pre_trained_tokenizer=False, randomly initializing Tokenizer architecture"
                )
            import json
    ...
    def train_basemodel_phase(self):
        ...
        if getattr(self.config, "pre_trained_tokenizer", True):
            ...
            if self.rank == 0:
                print(
                    "pre_trained_tokenizer=False, randomly initializing Tokenizer architecture for Predictor training"
                )
            import json
        ...
        if getattr(self.config, "pre_trained_predictor", True):
            ...
            if self.rank == 0:
                print(
                    "pre_trained_predictor=False, randomly initializing Predictor architecture"
                )
            import json
...
def main():
    ...
    parser.add_argument(
        "--skip-tokenizer", action="store_true", help="Skip tokenizer training phase"
    )
    parser.add_argument(
        "--skip-basemodel", action="store_true", help="Skip basemodel training phase"
    )
    parser.add_argument(
        "--skip-existing", action="store_true", help="Skip training for existing models"
    )
