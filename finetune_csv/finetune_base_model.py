...
class CustomKlineDataset(Dataset):
    def __init__(
        ...
        print(
            f"[{data_type.upper()}] Data length: {len(self.data)}, Available samples: {self.n_samples}"
        )
    ...
    def _load_and_preprocess_data(self):
        ...
        print(
            f"Original data time range: {self.timestamps.min()} to {self.timestamps.max()}"
        )
    ...
    def _split_data_by_time(self):
        ...
        if self.data_type == "train":
            ...
            print(
                f"[{self.data_type.upper()}] Training set: first {train_end} time points ({self.train_ratio})"
            )
            print(
        ...
        print(
            f"[{self.data_type.upper()}] Data length after split: {len(self.data)} records"
        )
...
def train_model(model, tokenizer, device, config, save_dir, logger):
    ...
    for epoch in range(config.basemodel_epochs):
        ...
        for batch_idx, (batch_x, batch_x_stamp) in enumerate(train_loader):
            ...
            ).head.compute_loss(logits[0], logits[1], token_out[0], token_out[1])

            optimizer.zero_grad()
            loss.backward()
            torch.nn.utils.clip_grad_norm_(
                (model.module if use_ddp else model).parameters(), max_norm=3.0
            )
            optimizer.step()
            scheduler.step()
        ...
        if use_ddp:
            ...
            avg_train_loss = (
                (epoch_train_loss_all / train_batches_all)
                if train_batches_all > 0
                else 0.0
            )
            avg_val_loss = (
                (val_loss_all / val_batches_all) if val_batches_all > 0 else 0.0
            )
        else:
            avg_train_loss = (
                epoch_train_loss / train_batches if train_batches > 0 else 0
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Kronos Basemodel Fine-tuning Training"
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    if getattr(config, "pre_trained_tokenizer", True):
        ...
        print(
            "pre_trained_tokenizer=False, randomly initializing Tokenizer architecture for training"
        )
    ...
    if getattr(config, "pre_trained_predictor", True):
        ...
        print(
            "pre_trained_predictor=False, randomly initializing Predictor architecture for training"
        )
    ...
    best_val_loss = train_model(
        model, tokenizer, device, config, config.basemodel_save_path, logger
    )
