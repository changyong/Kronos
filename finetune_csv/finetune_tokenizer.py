    ...
    for epoch in range(config.tokenizer_epochs):
        ...
        for batch_idx, (ori_batch_x, _) in enumerate(train_loader):
            ...
            torch.nn.utils.clip_grad_norm_(
                (model.module if use_ddp else model).parameters(), max_norm=2.0
            )
            optimizer.step()
            scheduler.step()
            optimizer.zero_grad()
            ...
            if (batch_idx_global + 1) % config.log_interval == 0:
                ...
                log_msg = (
                    f"[Epoch {epoch+1}/{config.tokenizer_epochs}, Step {batch_idx+1}/{len(train_loader)}] "
        ...
        if use_ddp:
            ...
            avg_val_loss = (
                (tot_val_loss_all / val_count_all) if val_count_all > 0 else 0.0
            )
        else:
            avg_val_loss = (
                tot_val_loss_sum_rank / val_sample_count_rank
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Kronos Tokenizer Fine-tuning Training"
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    best_val_loss = train_tokenizer(
        tokenizer, device, config, config.tokenizer_save_path, logger
    )
